...
class _CarritoPageState extends State<CarritoPage> {
      ...
      appBar: AppBar(
        ...
        TextButton(
               onPressed: () {
                setState(() {
                  cajeros.clear();
                  _carritoBloc.eliminarProductos();
                  _catalogoBloc.listarPromociones(_carritoBloc.promociones.first.idAgencia.toString());
                  _carritoBloc.promociones.clear();
  ...
  cambiarDireccion({bool mostrarResumen: true}) async {
    ...
    List<String> agencias = promociones.length>0 ? [promociones.first.idAgencia.toString()] : [];
    cajeros = await _cajeroProvider.verCostoPromocion(
        config.COMPRA_TIPO_CATALOGO,
        direccionSeleccionada,
        promociones.first.idAgencia.toString(),
  ...
  int _total_tiempo_preparacion = 0;

  evaluarCosto() async {
    costoTotal = 0.0;
    _cantidadProductos = 0;
    _total_tiempo_preparacion = 0;
    await cargarPromociones();
    final idsAgencias = [];
    ...
    promociones.forEach((promocion) {
      ...
      _cantidadProductos++;
      idsAgencias.add(promocion.idAgencia.toString());
      _total_tiempo_preparacion = _total_tiempo_preparacion + (int.parse(promocion.tiempo_preparacion)*promocion.cantidad);
    });
    //List<String> agencias = LinkedHashSet<String>.from(idsAgencias).toList();
    List<String> agencias = promociones.length>0 ? [promociones.first.idAgencia.toString()] : [];
    if (direccionSeleccionada.idDireccion > 0 && cajeros != null) {
      ...
      for (var item in cajeros) {
        if (agencias.contains(item.idAgencia.toString())) {
          ...
          _total_tiempo_preparacion = _total_tiempo_preparacion + item.tiempo_viaje;
          cajerosAux.add(item);
        }
      }
  ...
  _onselecDireccion(DireccionModel direccion) async{
    Navigator.pop(context);
      ...
      return StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<double> snapshot) {
          if (snapshot.hasData) {
            ...
            return btn.bootonContinuar('↑Selecciona una dirección de entrega↑', _mostrarDirecciones);
          }
        },
      ); */
    return btn.bootonContinuar('↑ Selecciona una direccion de entrega ↑', (){
      _mostrarDirecciones();
    });
  }
    ...
    showDialog(
        ...
        builder: (context) {
          return CarritoDialog(config.COMPRA_TIPO_CATALOGO,
              ...
              total_tiempo_preparacion: _total_tiempo_preparacion,
              costoEnvio: cajeros.first.costoEnvio,);
        });
  }
