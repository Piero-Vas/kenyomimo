...
class _ChatDespachoPageState extends State<ChatDespachoPage>
  ...
  void initState() {
    ...
    FirebaseFirestore.instance.collection("despacho_chat").where("id_despacho",isEqualTo: despachoModel.idDespacho)..orderBy("id_chat",descending: true).snapshots().listen((event) {
      despachoModel.idDespachoEstado = event.docs.first.get("id_despacho_estado");
      _chatDespachoBloc.obtener(despachoModel);
    });

    /* _pushProvider.estadosDespacho.listen((ChatDespachoEstadoModel chatDespachoEstadoModel) {
      if (!mounted) return;
      _chatDespachoBloc.chats.forEach((chatsCajeros) {
  ...
  Widget _avatar() {
    return Container(
      ...
      child: Column(
        children: <Widget>[
          CircleAvatar(
            backgroundImage: NetworkImage('${cache.img(despachoModel.img)}',),
            radius: 17.0,
          ),
