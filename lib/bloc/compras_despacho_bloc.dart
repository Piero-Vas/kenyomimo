...
class ComprasDespachoBloc {
  ...
  Future listarCompras(int tipo, String fecha, Position posConductor) async {
    if(tipo==0 && posConductor==null) return;
    List<DespachoModel> comprasResponse = await _despachoProvider.listarCompras(tipo, fecha);
    if(tipo==0 && posConductor!=null){
      List<DespachoModel> comprasTemp = comprasResponse;
      comprasResponse = await comprasTemp.where((DespachoModel despachoModel) {
        
        return despachoModel.ltA + 0.0430000 > posConductor.latitude && posConductor.latitude>despachoModel.ltA - 0.0430000 &&
                despachoModel.lgA + 0.0430000 >posConductor.longitude && posConductor.longitude > despachoModel.lgA - 0.0430000
            ? true
            : false;
      }).toList();
    }
