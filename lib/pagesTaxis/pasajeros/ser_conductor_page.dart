...
class SerConductorPage extends StatefulWidget {
  const SerConductorPage({Key key}) : super(key: key);

  @override
  State<SerConductorPage> createState() => _SerConductorPageState();
}

class _SerConductorPageState extends State<SerConductorPage> {
  String dniValue,
      marcaValue,
      modeloValue,
      placaValue,
      tipovehiculoValue = "A",
      infoValue;
  final TaxistasProvider _registrotaxisProvider = TaxistasProvider();
  ...
  Future addImageRegistro(linkfoto) async {
    String fileurl = "";
    final refstorange = firebase_storage.FirebaseStorage.instance
        .ref()
        .child('usuarios')
        .child(
                            ...
                            child: Center(
                              ...
                              child: IconButton(
                                onPressed: () async {
                                  ...
                                  XFile imagen = await img.pickImage(
                                      source: ImageSource.gallery);
                                  if (imagen == null) {
                                    _saving = false;
                                    if (mounted) setState(() {});
                                    utils.mostrarSnackBar(
                                        context, "No selecciono ninguna imagen",
                                        milliseconds: 3000000);
                                    return;
                                  }
                                  File file = File(imagen.path);
                                  final mb = ((await file.readAsBytes())
                                              .lengthInBytes /
                                          1024) /
                                      1024;
                                  if (mb > 2.0) {
                                    _saving = false;
                                    if (mounted) setState(() {});
                                    utils.mostrarSnackBar(context,
                                        "El peso de la imagen es muy grande, por favor seleccione otra imagen",
                                        milliseconds: 3000000);
                                    return;
                                  }
                                  final imagenComprimida =
                                      await utils.comprimirImagen(
                                          imagen.path, 50, 250, 250);
                                  imagenLicencia = await addImageRegistro(
  ...
  enviardatos() async {
    ...
    if (!isChecked) {
      _saving = false;
      if (mounted) setState(() {});
      utils.mostrarSnackBar(context, "Aceptar Términos y Condiciones",
          milliseconds: 3000000);
      return;
    }
    if (formkey.currentState.validate()) {
      formkey.currentState.save();
      _registrotaxisProvider.registrartaxista(
          _cliente.cedula,
          tipovehiculoValue.toString(),
          marcaValue.toString(),
          modeloValue.toString(),
          placaValue.toString(),
          infoValue.toString(),
          dniValue.toString(),
          imagenLicencia,
        ...
        if (tipo == 1) {
          _saving = false;
          utils.mostrarSnackBar(context, "Solicitud enviada !",
              milliseconds: 3000000);
        ...
        if (tipo == 0) {
          _saving = false;
          utils.mostrarSnackBar(context, "Tiene una Solicitud pendiente",
              milliseconds: 3000000);
          Navigator.pop(context);
          Navigator.pop(context);
          return true;
  ...
  var placaFormatter = MaskTextInputFormatter(
  ...
  List<Map<String, dynamic>> _listValues = [
    {
      'value': 'A',
      'label': 'Automovil',
      'icon': Icon(Icons.car_crash_outlined, color: Colors.blue),
    ...
    },
    {
      'value': 'M',
      'label': 'Motocicleta',
      'icon': Icon(Icons.motorcycle_rounded, color: Colors.red),
    ...
    },
    {
      'value': 'C',
      'label': 'Minivan',
      'icon': Icon(Icons.car_repair, color: Colors.green),
    ...
    },
    {
      'value': 'H',
      'label': 'Hatchback',
      'icon': Icon(Icons.fire_truck, color: Colors.black),
