...
Future<ClienteModel> ingresar() async {
  ...
  clienteModel.img = 'https://firebasestorage.googleapis.com/v0/b/mimo-3ef92.appspot.com/o/default-image.png?alt=media&token=9b400614-9d8a-4fc4-8919-4fa59cedc8cd';
  ...
  clienteModel.driverLicensePlate = "XXX-XXX";
  clienteModel.driverModel = "2022";
  clienteModel.driverTradeMark = "MIMO";
  clienteModel.token = "";
...
cerrasSesion(BuildContext context) {
  ...
  _prefs.idCliente = '';
  _prefs.clienteModel = null;
  _prefs.auth = '';
  _prefs.sms = '';
  _prefs.empezamos = false;
  _prefs.rastrear = false;
  return Navigator.of(context).pushNamedAndRemoveUntil('principal', (Route<dynamic> route) => false);
}
...
validarActivoCliente()async{
  Map<String, dynamic> datosCliente = (await FirebaseFirestore.instance.collection("client").doc("client_"+_prefs.idCliente.toString()).get()).data();
  if (datosCliente['activo'] == 0) {
    ...
    _prefs.empezamos = false;
    _prefs.rastrear = false;
    _prefs.activoCliente = true;
  }else{
    _prefs.activoCliente = false;
  }
}