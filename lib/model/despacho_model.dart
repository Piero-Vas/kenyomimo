...
class DespachoModel {
  ...
  String efectivoProdcuto() {
    return ((costo - credito) - (costoEnvio - creditoEnvio)).toStringAsFixed(2);
  }
  ...
  get label {
    ...
    if (idDespacho == 0) return 'Solicitud en Proceso';
  ...
  get _parse {
    ...
    dataSp.forEach((element) => mapData[element.split(':').first] = element.split(':').last);
    Map<String,String> mapData2 = {"aA":"aAA","a":"aAA"};
  ...
  get latLngBounds {
    ...
    if (double.parse(ltA.toString()) <= double.parse(ltB.toString())) {
      slt = double.parse(ltA.toString());
      nlt = double.parse(ltB.toString());
    } else {
      slt = double.parse(ltB.toString());
      nlt = double.parse(ltA.toString());
    }
    if (double.parse(lgA.toString()) <= double.parse(lgB.toString())) {
      slg = double.parse(lgA.toString());
      nlg = double.parse(lgB.toString());
    } else {
      slg = double.parse(lgB.toString());
      nlg = double.parse(lgA.toString());
    }
    return LatLngBounds(
