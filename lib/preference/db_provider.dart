...
class DBProvider {
  ...
  agregarPromocion(PromocionModel pr, {LP producto}) async {
    ...
    if (producto == null) {
      res = await db.rawInsert(
          "INSERT Into Promocion (id_agencia, id_promocion, id_producto, id_urbe, incentivo, producto, descripcion, precio, imagen, minimo, maximo, cantidad, costoTotal, dt, tiempo_preparacion) "
          "VALUES ( ${pr.idAgencia}, '${pr.idPromocion}', '${pr.idProducto}', '${pr.idUrbe}', '${pr.incentivo}','${pr.producto}','${pr.descripcion}','${pr.tieneDescuento ? pr.precioPromocion : pr.precio}','${pr.imagen}','${pr.minimo}','${pr.maximo}','${pr.cantidad}','${pr.tieneDescuento ? pr.costoTotalDescuento : pr.costoTotal}', '' ,'${pr.tiempo_preparacion}')");
    } else
      res = await db.rawInsert(
          "INSERT Into Promocion (id_agencia, id_promocion, id_producto, id_urbe, incentivo, producto, descripcion, precio, imagen, minimo, maximo, cantidad, costoTotal, dt, tiempo_preparacion) "
          "VALUES ( ${pr.idAgencia}, '${pr.idPromocion}', '${pr.idProducto}', '${pr.idUrbe}', '${pr.incentivo}','${pr.producto} (${producto.d})','${pr.descripcion}','${producto.p}','${pr.imagen}','${pr.minimo}','${pr.maximo}','${pr.cantidad}','${pr.tieneDescuento ? pr.costoTotalDescuento : pr.costoTotal}', '' ,'${pr.tiempo_preparacion}')");
  ...
  Future<List<PromocionModel>> search(dynamic idAgencia) async {
    final db = await database;
    final res = await db
