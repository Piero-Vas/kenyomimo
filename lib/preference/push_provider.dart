...
import 'package:mimo/pages/delivery/compras_despacho_page.dart';
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  obtenerToken() async {
    ...
    _prefs.token = await _firebaseMessaging.getToken();
    
    if (_prefs.idCliente == '' || _prefs.idCliente == Sistema.ID_CLIENTE) {
      return;
    } 
  ...
  Future _showNotification(RemoteNotification push) async {
    ...
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails(
            'Mimo_CHANNEL_ID', 'Mimo Notification',
            playSound: true,
            importance: Importance.max,
            priority: Priority.high,
            groupKey: 'Mimo_GROUP_KEY',
            autoCancel: true);
  ...
  initNotifications() {
    final initializationSettingsAndroid =
        AndroidInitializationSettings('launcher_icono');
  ...
  Future _onMessageOpenApp(RemoteMessage message) async {
    ...
    if (Platform.isAndroid) {
      ...
      else if(message.data['PUSH'] == '1122'){
        Navigator.pushNamedAndRemoveUntil(context, 'compras_despacho', (route) => false);
          // _prefs.dirigible = 1;
          // Navigator.pushNamedAndRemoveUntil(context, 'seleccion', (route) => false);
        
      }else if(message.data['PUSH'] == '1111'){
